...
Begin VB.Form frmZomBits 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "BitFight! - ZomBits"
   ClientHeight    =   8364
   ClientLeft      =   36
   ClientTop       =   384
   ClientWidth     =   17052
   Icon            =   "BitFight! - ZomBits.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   Picture         =   "BitFight! - ZomBits.frx":058A
   ScaleHeight     =   8364
   ScaleWidth      =   17052
   ...
   Begin VB.Timer timWarpMineBit 
      ...
      Top             =   120
   End
   Begin VB.Timer timZomBitsPosCheck 
      Interval        =   1
      Left            =   3240
      Top             =   120
   End
   Begin VB.Image Explosion1 
      Height          =   996
      Left            =   4920
      Picture         =   "BitFight! - ZomBits.frx":1F204
      Stretch         =   -1  'True
      Top             =   480
      Visible         =   0   'False
      Width           =   996
   ...
   Begin VB.Image TopFire1 
      Height          =   300
      Left            =   6480
      Picture         =   "BitFight! - ZomBits.frx":1F69B
      Stretch         =   -1  'True
      Top             =   480
      Width           =   300
   End
   ...
   Begin VB.Image RightFire1 
      Height          =   300
      Left            =   6840
      Picture         =   "BitFight! - ZomBits.frx":1FAF8
      Stretch         =   -1  'True
      Top             =   840
      Width           =   300
   End
   Begin VB.Image DownFire1 
      Height          =   300
      Left            =   6480
      Picture         =   "BitFight! - ZomBits.frx":1FEEF
      Stretch         =   -1  'True
      Top             =   1200
      Width           =   300
   End
   Begin VB.Image LeftFire1 
      Height          =   300
      Left            =   6120
      Picture         =   "BitFight! - ZomBits.frx":202D7
      Stretch         =   -1  'True
      Top             =   840
      Width           =   300
   End
...
Private Sub cmdReset_Click()
    ...
    If cmdReset.Caption = "Go to Survival!" Then
    
        MsgBox ("Loading Survival: Easy... Good Luck " & PlayerName & "!")
        Unload Me
...
Private Sub Form_Load()
    ...
    txtSystemLog.Text = " - LOG INITIALIZED - " _
                            ...
                            + vbNewLine _
                            + vbNewLine _
                            + ">>> Difficulty: ZomBits" _
                            + vbNewLine _
                            + vbNewLine _
                            + ">>> OBJECTIVE: Survive. Just Survive..." _
                            + vbNewLine _
                            + vbNewLine _
                            + ">>> You will get 1 Point for every Enemy-Wall Collision, and Point Multipliers will be added for getting consecutive Power Ups that appear on the Arena... Your job is to avoid the ZomBits..." _
                            + vbNewLine _
                            + vbNewLine _
    ...
    'Position of End Wall
    ZomBitsEndWall.X1 = ZomBitEndWallX
    ZomBitsEndWall.Y1 = ZomBitEndWallY
    ZomBitsEndWall.X2 = ZomBitEndWallX
    ZomBitsEndWall.Y2 = 6120
...
End Sub
Private Sub timZomBitsPosCheck_Timer() 'Keeps checking and Updating the Positions if not enabled
    ...
    ElseIf ZomBitsProgression = 1 Then
    ...
    ElseIf ZomBitsProgression = 2 Then
    ...
    ElseIf ZomBitsProgression = 3 Then
    ...
    ElseIf ZomBitsProgression = 4 Then
    ...
    ElseIf ZomBitsProgression = 5 Then
    ...
    ElseIf ZomBitsProgression = 6 Then
...
Private Sub timEnemyMotion_Timer() 'Makes Enemies move, depending on Points and Progression
    ...
    Call checkEnemyZomBitsWallCollision(ZomBitsProgression) 'Checks Wall Collisions
...
Private Sub timgamemechanics_Timer()
    ...
    'Checks ZomBitsAiming
    Call checkZomBitsAiming
    ...
    If PowerUpBit.Visible = True Then
        ...
        If (PowerUpBitNumber) = 2 Or (PowerUpBitNumber) = 30 Then 'Checks ID of Power Up (SLOW MO FILL UP)
            ...
            If Abs(UserBitX - PowerUpBitX) < 300 And Abs(UserBitY - PowerUpBitY) < 300 Then
                ...
                Call ZomBitsTargetUser(UserBitY, EnemyBit1Y, EnemyBit1YSpeed, EnemyBit1XSpeed)
                Call ZomBitsTargetUser(UserBitY, EnemyBit2Y, EnemyBit2YSpeed, EnemyBit2XSpeed)
                Call ZomBitsTargetUser(UserBitY, EnemyBit3Y, EnemyBit3YSpeed, EnemyBit3XSpeed)
                Call ZomBitsTargetUser(UserBitY, EnemyBit4Y, EnemyBit4YSpeed, EnemyBit4XSpeed)
                Call ZomBitsTargetUser(UserBitY, EnemyBit5Y, EnemyBit5YSpeed, EnemyBit5XSpeed)
                Call ZomBitsTargetUser(UserBitY, EnemyBit6Y, EnemyBit6YSpeed, EnemyBit6XSpeed)
        ...
        ElseIf (PowerUpBitNumber) = 14 Or (PowerUpBitNumber) = 19 Then 'Checks ID of Power Up (LEVEL MODIFIER)
            ...
            If Abs(UserBitX - PowerUpBitX) < 300 And Abs(UserBitY - PowerUpBitY) < 300 Then
                ...
                WallNumber = CInt(Int((2 - 1 + 1) * Rnd() + 1))
                
                'These Call the Movement in wall, and then changes positions if necessary
                Call ZomBitsWallModifier(WallNumber)
                Call ZomBitWallBitPosition(WallNumber)
...
Private Sub timPowerUpResultant_Timer()
    ...
    If isWall = True Then 'if there is a Wall
        ...
        frmZomBits.Refresh 'Refreshes Form
...
Private Sub timZomBits_Timer()
    ...
    If (TotalPoints > 30999) Then 'Checks if Player has achieved the necessary 'win' conditions (To stop Overflow).
        ...
        timZomBits.Enabled = False
        timWarpMineBit.Enabled = False
        timZomBitsPosCheck.Enabled = False
        ...
        lblGameOver.Caption = "You have beaten ZomBits! You have completely finished ZomBits, but have you done Survival?"
        
        If Explosion1.Left > 7000 Then
        
            fraRestart.Left = 700
            cmdReset.Caption = "Go to Survival!"
            cmdReset.SetFocus
...
Private Sub timUserMotion_Timer()
    ...
    Call checkUserZomBitWallCollision
